...
const StereographicShader = {
    ...
    vertexShader: [
        ...
        'vUv = uv;',
        'gl_Position = vec4( position, proj_1.0 );',
    ...
    fragmentShader: [
        ...
        'vec2 position = -proj_1.0 +  2.0 * vUv;',

        'position *= vec2( zoom * resolution, zoom * 0.5 );',

        'float x2y2 = position.x * position.x + position.y * position.y;',
        'vec3 sphere_pnt = vec3( 2. * position, x2y2 - proj_1. ) / ( x2y2 + proj_1. );',

        'sphere_pnt = vec3( transform * vec4( sphere_pnt, proj_1.0 ) );',

        'vec2 sampleUV = vec2(',
        '(atan(sphere_pnt.y, sphere_pnt.x) / PI + proj_1.0) * 0.5,',
